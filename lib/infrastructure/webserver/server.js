...
io.on("connection", async (socket) => {
  ...
  socket.on(process.env.JOIN_TO_ROOM, (roomId, cb) => {
    return socketRouter.joinToRoom(roomId, socket.userId).then((response) => {
      if (response.status) {
        socket.join(roomId);
        socket.to(roomId).emit(process.env.NEW_USER_JOIN, { some: "a new user has joined the room" });
      }
      cb(response);
    });
  })
  ...
  socket.on(process.env.LEAVE_ROOM, (roomId, cb) => {
    return socketRouter.leaveRoom(roomId, socket.userId).then(async (response) => {
      if (response.status) {
        socket.leave(roomId);
        socket.to(roomId).emit(process.env.NEW_USER_JOIN, { some: `user ${socket.id} has left the room` });
      }
      socket.emit(process.env.LIST_ROOMS, await socketRouter.listRooms())
      cb(response);
    })
  ...
  socket.on(process.env.START_GAME, (roomId, coin, cb) => {
    return socketRouter.startGame(roomId, coin).then((response) => {
      socket.in(roomId).emit(process.env.START_GAME_SEND_INFO, response);
      cb(response);
    })
  })
  ...
  socket.on(process.env.NEXT_USER_DRAW_CARDS, (matchId, roomId, cb) => {
    return socketRouter.getUserDrawCards(matchId, socket.userId).then((response) => {
      console.log(roomId);
      socket.in(roomId).emit(process.env.NEXT_PLAYER, response);
      cb(response);
    })
  })
